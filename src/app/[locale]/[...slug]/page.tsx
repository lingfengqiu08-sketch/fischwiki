...
export async function generateMetadata({ params }: { params: Promise<{ locale: Locale; slug: string[] }> }): Promise<Metadata> {
  ...
  if (slug.length === 1 && CONTENT_TYPES.includes(slug[0])) {
    ...
    const title = ctMessages?.overviewTitle || `${ctTitle} — Fisch Wiki`;
    const description = ctMessages?.overviewDescription || `Browse all ${ctTitle.toLowerCase()} guides and resources for Fisch.`;
  ...
  return { title: `${item.metadata.title} — Fisch Wiki`, description: item.metadata.description, alternates: { canonical: localizedPathname, languages: languageAlternates(pathname) }, openGraph: { type: "article", title: item.metadata.title, description: item.metadata.description, url: `${siteUrl}${localizedPathname}`, images: [image] }, twitter: { card: "summary_large_image", images: [image] } };
}
...
async function NavigationPage({ locale, contentType, navGroups }: { locale: Locale; contentType: string; navGroups: import("@/lib/content").NavGroup[] }) {
  ...
  const listData = { "@context": "https://schema.org", "@type": "ItemList", name: `${contentType} — Fisch Wiki`, itemListElement: items.map((item, index) => ({ "@type": "ListItem", position: index + 1, url: `${siteUrl}/${contentType}/${item.slug}`, name: item.metadata.title })) };
